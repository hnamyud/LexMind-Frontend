...
export default function LawDetailPanel({ nodeId, onClose }: LawDetailPanelProps) {
    ...
    return (
        <>
            ...
            {/* Drawer panel */}
            <aside
                className={`fixed top-0 right-0 h-full z-50 flex flex-col bg-[#0d0d0d] border-l border-gray-800 shadow-2xl shadow-black/50 transition-transform duration-300 ease-in-out ${
            ...
            >
                ...
                {/* Body – scrollable */}
                <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4 law-panel-scroll">
                    {loading && (
                        <div className="flex flex-col items-center justify-center py-16 gap-3">
                            <div className="w-8 h-8 border-2 border-brand/30 border-t-brand rounded-full animate-spin" />
                            <span className="text-xs text-gray-500 uppercase tracking-widest">Đang tải...</span>
                        </div>
                    )}
                    ...
                    {!loading && !error && lawNodes.length === 0 && isOpen && (
                        <div className="flex flex-col items-center justify-center py-16 gap-2 text-center">
                            ...
                            <p className="text-xs text-gray-500">Không tìm thấy nội dung điều luật.</p>
                        </div>
                    )}
...
function LawNodeCard({ node, index, total }: { node: LawNode; index: number; total: number }) {
    ...
    return (
        ...
        >
            ...
            {depth > 0 && (
                <div
                    className="absolute top-0 -left-3 w-3 h-4 border-l border-b border-gray-700"
            ...
            <div className="px-4 py-3">
                ...
                <p className={`text-[13px] font-semibold leading-relaxed mb-2 ${
                    depth === 0 ? "text-brand" : "text-gray-200"
                }`}>
                    {node.text}
                </p>

                {/* Full legal text */}
                <div className="text-[12px] text-gray-400 leading-relaxed border-t border-gray-800/60 pt-2 whitespace-pre-wrap">
                    {node.raw_text}
                </div>
            </div>
        </div>
